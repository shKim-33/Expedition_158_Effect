...
REGISTER_GLOBAL_ENUM(Client::ScreenFxShapeMode, "ScreenFxShapeMode")

IMPLEMENT_REFLECTION(Flash_ScreenFx)
{
    auto& info = GetStaticReflectionInfo();
    info.properties.clear();
    info.displayName = "Flash";
    info.type = RegisteredType::ScreenFx;

    PROPERTY_COLOR("Color", _originalDesc.color);
    PROPERTY_FLOAT("Intensity", _originalDesc.intensity, 0.f, 10.f, 0.01f);
...
HRESULT Flash_ScreenFx::Initialize(ScreenFxDesc* desc)
{
    if (desc)
    {
        ...
        const FlashDesc* flashDesc = static_cast<FlashDesc*>(desc);
        _originalDesc = *flashDesc;
    }
    else
    {
    ...
    _passType = ETOI(_originalDesc.exclusionMode == ScreenFxExclusionMode::MarkedObjects
        ? ClientScreenFxManager::ScreenFx_Type::FlashExclude
        : ClientScreenFxManager::ScreenFx_Type::Flash);
    Evaluate_At(_elapsedTime);
    return S_OK;
}

void Flash_ScreenFx::Update_ScreenFx(float timeDelta)
{
    _elapsedTime += timeDelta;
    Evaluate_At(_elapsedTime);
}

HRESULT Flash_ScreenFx::Bind_FxConstantBuffer(ShaderCom* shader)
{
    Engine::ScreenFxCB cbData{};

    cbData.flashColor = _originalDesc.color;
    ...
    cbData.screenFxParams = Vec4(
        ...
        max(0.f, _originalDesc.intensity));

    return shader->Bind_CBufferData(cbData);
}

void Flash_ScreenFx::Evaluate_At(float elapsedTime)
{
    ...
    _currentAmount = std::clamp(Compute_Envelope(_elapsedTime) * _originalDesc.color.w, 0.f, 1.f);
}

float Flash_ScreenFx::Compute_Envelope(float elapsedTime) const
{
...
Shared<Flash_ScreenFx> Flash_ScreenFx::Create(FlashDesc* desc)
{
    auto instance = make_shared<Flash_ScreenFx>();
