...
cbuffer cbEffectMeshGlassMaterial : register(b2)
{
...
VS_OUT VS_MAIN(VS_IN In)
{
    ...
    const float4 worldPosition = mul(float4(In.vPosition, 1.f), mPreWorld);
...
PS_OUT PS_GLASS(VS_OUT In)
{
    ...
    const float3 tintBase = g_Tint.rgb * In.vInstanceColor.rgb;
    const float3 patternedTint = lerp(tintBase, tintBase * mainTexel.rgb, saturate(g_EffectMeshGlassSurfaceParams.w));
    ...
    const float3 emissive = g_EffectMeshModelMaterialFlags.z > 0.5f
                            ...
                            : float3(0.f, 0.f, 0.f);
    const float alpha = saturate((g_EffectMeshGlassSurfaceParams.x + fresnel * 0.35f + specular * 0.25f) * coverage * g_Tint.a * In.vInstanceColor.a);
